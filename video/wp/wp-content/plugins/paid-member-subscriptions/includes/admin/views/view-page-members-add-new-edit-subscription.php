									...
									<div id="major-publishing-actions">
										...
										<?php if( $subpage == 'edit_subscription' ): ?>
											<div id="delete-action">
												<a href="<?php echo wp_nonce_url( add_query_arg( array( 'subscription_id' => $form_data['id'] ), admin_url( 'admin.php?page=pms-members-page' ) ), 'pms_delete_subscription_nonce' ) ?>" class="submitdelete deletion" onclick="return confirm( '<?php _e( 'Are you sure you want to delete this Subscription? \nThis action is irreversible.', 'paid-member-subscriptions' ); ?>' )"><?php _e( 'Delete Subscription', 'paid-member-subscriptions' ); ?></a>
											</div>
										<?php endif; ?>
									...
									<?php elseif( pms_payment_gateways_support( array( $member_subscription->payment_gateway ), 'change_subscription_payment_method_admin' ) ) : ?>
										...
										if( pms_is_payment_retry_enabled() ) {
											...
											if( $subscription_payment_retry == 'active' ) {
												?>
												<div class="pms-meta-box-field-wrapper">
													<label class="pms-meta-box-field-label"><?php _e( 'Payment Retry', 'paid-member-subscriptions' ); ?></label>

													<span class="readonly medium"><strong><?php _e( 'Active', 'paid-member-subscriptions' ); ?></strong></span>

													<p class="description"><?php printf( __( 'A new payment attempt will be made on %s. After %s more attempts, the subscription will remain expired.', 'paid-member-subscriptions' ), '<strong>' . $member_subscription->billing_next_payment . '</strong>', '<strong>' . ( ( (int)apply_filters( 'pms_retry_payment_count', 3 ) - pms_get_subscription_payments_retry_count( $member_subscription->id ) ) + 1 ) . '</strong>' ); ?></p>
												</div>
												<?php
											}
