...
<div id="payments-general">
    ...
    <div class="pms-form-field-wrapper">
        <label class="pms-form-field-label" for="payment-currency-position"><?php _e( 'Price Display Format', 'paid-member-subscriptions' ) ?></label>
        ...
        <p class="description"><?php _e( 'Select how prices should be displayed.', 'paid-member-subscriptions' ); ?></p>
    </div>
    ...
    if( count( $payment_gateways ) > 1 ) :
        ...
        if( pms_payment_gateways_support( pms_get_payment_gateways( true ), 'plugin_scheduled_payments' ) ) : ?>
            ...
            <div class="pms-form-field-wrapper">
                <label class="pms-form-field-label" for="retry-payments"><?php _e( 'Retry Payments' , 'paid-member-subscriptions' ) ?></label>

                <p class="description">
                    <input type="checkbox" id="retry-payments" name="pms_payments_settings[retry-payments]" value="1" <?php echo ( isset( $this->options['retry-payments'] ) ? 'checked' : '' ); ?> /><?php _e( 'Enable', 'paid-member-subscriptions' ); ?>
                </p>
                <p class="description">
                    <?php printf( __( 'By checking this option, if a payment fails, the plugin will try to charge the user again after %s days for a maximum of %s retries.', 'paid-member-subscriptions' ), 3, 3 );//@TODO: apply filter ?><br>
                    <?php _e( 'This is valid for the Stripe and PayPal Express with Reference Transactions payment gateways. For PayPal Subscriptions, this is happening by default.', 'paid-member-subscriptions' ); ?>
                </p>
            </div>
